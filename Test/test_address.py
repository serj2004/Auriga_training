...
def test_address_on_home_page(app):
    ...
    assert contact_address_from_home_page.address == merge_address_like_on_home_page(contact_address_from_edit_page)
...
def merge_address_like_on_home_page(contact):
    return " ".join(filter(lambda x: x != "", filter(lambda x: x is not None, [contact.address])))
